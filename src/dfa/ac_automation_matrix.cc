struct Trie {
  ...
  int newnode() {
    for (int i = 0; i < MAX_CHILD_NUM; i++) next[node_cnt][i] = -1;
    count[node_cnt++] = 0;
    return node_cnt - 1;
  }

  int get_id(char a) { return a - 'A'; }
  ...
  void debug() {
    for (int i = 0; i < node_cnt; i++) {
      ...
      for (int j = 0; j < MAX_CHILD_NUM; j++) printf("%2d", next[i][j]);
      printf("]\n");
    }
  }
} ac;
