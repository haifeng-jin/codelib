...
double cross_product(Point a, Point b) { return a.x * b.y - b.x * a.y; }
...
double point_dist(Point a) { return sqrt(a.x * a.x + a.y * a.y); }
...
struct Line {
  ...
  bool operator==(const Line &l) const { return l.a == a && l.b == b; }
};
...
bool same_side(Point p1, Point p2, Line l) {
  return double_cmp(cross_product(l.a, p1, l.b) * cross_product(l.a, p2, l.b)) >
         0;
}

bool point_online_inclusive(Point p, Line l) {
  return zero(cross_product(p, l.a, l.b)) &&
         double_cmp(dot_product(p, l.a, l.b)) <= 0;
}
...
bool overlap_exclusive(Line u, Line v) {
  if (u == v || (u.a == v.b && u.b == v.a)) return true;
...
bool is_convex(Polygon &a)  // point should be counter-clockwise
{
  for (int i = 0; i < a.point_num; i++) {
    ...
    if (double_cmp(cross_product(p1, p2)) < 0) return false;
  }
  return true;
}

bool point_in_convex(Polygon &a,
                     Point peg)  // point should be counter-clockwise
{
  for (int i = 0; i < a.point_num; i++) {
    Point p1 = a.point[(i + 1) % a.point_num] - a.point[i];
...
int graham(Point point[], int n, Point ret[]) {
  ...
  for (int i = 0; i < 2; i++) ret[top++] = point[i];

  if (n <= 2) return n;

  for (int i = 2; i < n; i++) {
    while (top >= 2 && graham_ok(ret[top - 1], ret[top - 2], point[i])) top--;
    ret[top++] = point[i];
  }
  int len = top;
  ret[top++] = point[n - 2];
  for (int i = n - 3; i >= 0; i--) {
    while (top > len && graham_ok(ret[top - 1], ret[top - 2], point[i])) top--;
    ret[top++] = point[i];
  }
  return top;
}
